...
export default function PpdbRegistrationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? <ErrorState message={error} onRetry={() => void refetchList()} title="Gagal memproses pendaftaran PPDB" /> : null}
      ...
      <SectionCard
        ...
        description={
          <>
            Daftar pendaftar PPDB. Total: <strong>{total}</strong> data.
          </>
        }
      ...
      {expandedId ? (
        <SectionCard description="Verifikasi dokumen, ubah status, dan konversi ke siswa." title="Detail Pendaftaran">
          {detailLoading ? (
            <div className="grid min-h-48 place-items-center rounded-xl border border-dashed bg-surface-muted text-sm font-bold text-muted-foreground">
              <span className="inline-flex items-center gap-2">
