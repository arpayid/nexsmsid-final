...
import { Badge, Button, ModuleCard, SectionCard } from "@nexsmsid/ui";
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10 px-4 py-12 sm:px-6 lg:px-8">
      <div className="text-center">
        ...
        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">Penerimaan Peserta Didik Baru</h1>
        <p className="mx-auto mt-5 max-w-2xl text-muted-foreground">
          Pendaftaran dilakukan secara daring. Lengkapi data, pilih jurusan, dan pantau status seleksi.
        </p>
      </div>

      <SectionCard title="Periode Pendaftaran Aktif">
        {ppdb ? (
            ...
            {ppdb.quota ? (
              <div>
                ...
                <div className="mt-2 h-2 rounded-full bg-muted">
                  <div className="h-2 rounded-full bg-emerald-500" style={{ width: `${ppdb.fillPercent ?? 0}%` }} />
                </div>
              </div>
            ) : (
      ...
      <SectionCard description="Tiga langkah utama pendaftaran daring." title="Alur Pendaftaran">
        <div className="grid gap-4 md:grid-cols-3">
          <ModuleCard
            description="Lengkapi data pribadi dan pilih jurusan."
            icon={<FileText className="h-5 w-5" />}
            title="Isi Data Diri"
            tone="emerald"
          />
          <ModuleCard
            description="Unggah berkas persyaratan melalui halaman cek status."
            icon={<School className="h-5 w-5" />}
            title="Upload Dokumen"
            tone="blue"
          />
          <ModuleCard
            description="Cek progres seleksi dengan nomor registrasi."
